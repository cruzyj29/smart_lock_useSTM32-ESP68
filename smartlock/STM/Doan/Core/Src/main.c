...
int main(void)
{
  ...
  while (1)
  {
		...
		/*if (count_mode%2==0){
		HAL_GPIO_WritePin(GPIOA,GPIO_PIN_3,1);//den bao bat tat 
			lcd_clear_display();
		lcd_goto_XY(1,0);
			
		lcd_send_string("CHE DO: TU DONG");
				lcd_goto_XY(2,0);
		...
		if (count_mode%2==1){
		HAL_GPIO_WritePin(GPIOA,GPIO_PIN_4,1);  //den bao bat tat 

		lcd_goto_XY(1,0);
		lcd_send_string("CHE DO: THU CONG");
	}
     ...
     HAL_GPIO_WritePin(GPIOA,GPIO_PIN_8,1); 
		 ...
		 if(rain <=1550 && light <= 2400) 
			 // ban ngay co mua. thuc hien dong cua  
		 {      lcd_goto_XY(2,12);
				lcd_send_string("DONG");
			      HAL_GPIO_WritePin(GPIOA,GPIO_PIN_11,1);
					  HAL_GPIO_WritePin(GPIOA,GPIO_PIN_12,1);
					  if(HAL_GPIO_ReadPin(GPIOA,GPIO_PIN_5)==1 && 
							HAL_GPIO_ReadPin(GPIOA,GPIO_PIN_6)==1&& 
						HAL_GPIO_ReadPin(GPIOA,GPIO_PIN_15)==0)
                           {Q2=0;}// kiem tra dong cua hay khong neu dong thi dong thi giu nguyen neu dang mo thi thuc hien qua  trinh dong 					      	
           	

if(HAL_GPIO_ReadPin(GPIOA,GPIO_PIN_5)==0 && 
HAL_GPIO_ReadPin(GPIOA,GPIO_PIN_6)==0&& 
HAL_GPIO_ReadPin(GPIOA,GPIO_PIN_15)==0 && Q1 ==0){Q2=0;}
	  ...
	  }   if(rain <= 1550 &&  light > 2400) // ban ngay khong mua. thuc hien mo 
			  {  
											 lcd_goto_XY(2,12);
												lcd_send_string("MO");
